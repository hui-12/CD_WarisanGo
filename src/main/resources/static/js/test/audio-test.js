document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      updateStatus(error.message || 'Processing failed.', 'danger');
    } finally {
  ...
  function resetResults() {
    ...
    if (audioFileContainer) audioFileContainer.classList.add('d-none');

    if (transcriptContainer) transcriptContainer.classList.add('d-none');

    if (jsonContainer) jsonContainer.classList.add('d-none');
