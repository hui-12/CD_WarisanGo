...
const initializeGuestGate = () => {
    ...
    (event) => {
      const action = event.target.closest(
        '.save-btn, [data-save-business], .map-saved-indicator, [data-check-in-business], ' +
          '.btn-popup-check-in, [data-challenge-action], [data-guest-gated], [data-member-required], ' +
  ...
  document.addEventListener(
    'submit',
    (event) => {
      if (!event.target.matches('[data-member-required-form]')) return;
      event.preventDefault();
      event.stopImmediatePropagation();
      guestGate.showModal();
    },
  ...
  guestGate.addEventListener('click', (event) => {
    if (event.target === guestGate) guestGate.close();
  });
};
