...
async function loadHomePoints() {
  ...
  const progressEl = document.getElementById('tier-progress-bar');

  const nextInfoEl = document.getElementById('tier-next-info');

  if (!tierNameEl || !pointsEl || !progressEl || !nextInfoEl) {
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Unable to load points. HTTP ${response.status}`);
    }
    ...
    console.error('Failed to load Firebase points:', error);
    ...
    nextInfoEl.textContent = 'Unable to load points';
  }
}
...
function renderHomeTier(points) {
  ...
  if (nextTier) {
    ...
    const progress = (pointsIntoTier / tierRange) * 100;

    progressEl.style.width = `${Math.max(0, Math.min(100, progress))}%`;

    const remaining = nextTier.min - points;

    nextInfoEl.textContent = `${remaining.toLocaleString()} pts to ${nextTier.name}`;
  } else {
    /*
    ...
    nextInfoEl.textContent = 'Highest tier reached';
  }
}
...
const renderVisits = async () => {
  ...
  try {
    const response = await fetch('/api/visits/this-week');
    ...
    visits.forEach((v) => {
      ...
      dateDiv.textContent = v.visitedAt
        ? new Date(v.visitedAt).toLocaleString('en-MY', {
            dateStyle: 'medium',
            timeStyle: 'short',
          })
        : '';
...
const renderActiveChallenges = async () => {
  ...
  try {
    ...
    if (!active.length) {
      widget.innerHTML =
        '<p class="widget-label">ACTIVE CHALLENGES</p>' +
    ...
    widget.innerHTML =
      '<p class="widget-label">ACTIVE CHALLENGES</p>' + '<p class="challenge-empty">Unable to load challenges.</p>';
  }
};
