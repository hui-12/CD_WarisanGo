(function () {
  ...
  const formatDate = (value) => (value ? new Date(value).toLocaleDateString('en-MY', { dateStyle: 'medium' }) : '');
  ...
  const renderStats = () => {
    ...
    document.getElementById('badge-stats').innerHTML = stats
      .map(
        (stat) => `
    ...
    document.querySelector('[data-badge-filter="locked"]').textContent = `Locked (${badges.length - earned})`;
  };
