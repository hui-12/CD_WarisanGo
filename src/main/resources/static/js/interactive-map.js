document.addEventListener('DOMContentLoaded', () => {
  ...
  const map = L.map('map-canvas').setView([3.8, 101.5], 7);
  ...
  const markSidebarCheckedInToday = (businessId) => {
    const card = Array.from(document.querySelectorAll('.location-item-card')).find(
      (item) => item.dataset.businessId === businessId
    );
  ...
  const populateStateDropdown = (items) => {
    ...
    const statesInDb = [...new Set(items.map((item) => item.state?.trim()).filter(Boolean))].sort((a, b) =>
      a.localeCompare(b)
    );
    ...
    const selectionStillExists = Array.from(stateSelect.options).some((option) => option.value === selectedState);
  ...
  const eventSource = new EventSource('/api/map/stream');

  eventSource.addEventListener('business-update', (event) => {
    try {
      const updatedLocations = JSON.parse(event.data);
      if (!Array.isArray(updatedLocations)) {
        throw new TypeError('Map update payload must be an array.');
      }
      ...
      console.warn('Ignored an invalid map update payload:', error);
    }
  });
