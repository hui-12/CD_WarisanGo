document.addEventListener('DOMContentLoaded', () => {
  ...
  function refreshCityOptions() {
    ...
    fillSelect(cityFilter, stateMatches.map(item => item.city), 'All Cities', selectedCity);
  }
  ...
  function render() {
    ...
    const displayed = businesses.filter(item => {
      ...
      return (!query || searchText.includes(query)) && (!state || item.state === state)
        && (!city || item.city === city);
    });
    ...
    results.innerHTML = displayed.map(item => {
  ...
  async function removeSavedListing(businessId) {
    ...
    fillSelect(stateFilter, businesses.map(item => item.state), 'All States', stateFilter.value);
    refreshCityOptions();
    render();
  }
  ...
  stateFilter.addEventListener('change', () => { refreshCityOptions(); render(); });
  ...
  fetch('/api/saved-listings').then(response => {
    ...
    summary.textContent = businesses.length
      ...
      : "You haven't saved any heritage businesses yet.";
    filters.hidden = businesses.length === 0;
    fillSelect(stateFilter, businesses.map(item => item.state), 'All States');
    refreshCityOptions();
    render();
  }).catch(() => {
